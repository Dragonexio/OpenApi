...
1. Response:
    ...
    | data.order_toggle | string | On |  | [开关](./枚举说明.md#开关) |
...
1. Response:
    ...
    | data.list[i].order_toggle | string | On |  | [开关](./枚举说明.md#开关) |
...
1. Params:
    ...
    | offset | int | 0 | N | 偏移量，从0开始，默认0 |
    | statuses | string | SettlementComplete,Listing | N | 要查询的[合约状态](./枚举说明.md#交割合约合约状态)，多个用`,`分隔 |
    | symbol | string | btc-usdt | N | 合约对应的交易对 |
...
1. Response:
    ...
    | data.contracts[i].contract_type | string | ThisWeek |  | [合约类型](./枚举说明.md#交割合约合约类型) |
    | data.contracts[i].delivery_date | int | 1578038400 |  | 交割日期, 秒级时间戳 |
    | data.contracts[i].listing_date | int | 1576828800 |  | 上市日期, 秒级时间戳 |
    | data.contracts[i].status | string | SettlementComplete |  | [合约状态](./枚举说明.md#交割合约合约状态) |
    ...
    | data.symbols[i].order_toggle | string | On |  | [下单开关](./枚举说明.md#开关) |
...
1. Params:
    ...
    | limit | int | 3 | N | 条数，最大500 |
    | offset | int | 0 | N | 偏移量，从0开始，默认0 |
    | order_sides | string |  | N | [订单状态](./枚举说明.md#交割合约订单状态)列表，多个用`,`分隔 |
    | order_types | string |  | N | [订单类型](./枚举说明.md#交割合约下单类型))列表，多个用`,`分隔 |
    | sort_field | string | UpdateTime | N | [查询结果排序方式](./枚举说明.md#查询结果排序方式) |
...
1. Params:
    ...
    | offset | int | 0 | N | 偏移量，从0开始，默认0 |
    | order_sides | string |  | N | [订单状态](./枚举说明.md#交割合约订单状态)列表，多个用`,`分隔 |
    | order_types | string |  | N | [订单类型](./枚举说明.md#订单类型)列表，多个用`,`分隔 |
    | sort_field | string | UpdateTime | N | [查询结果排序方式](./枚举说明.md#查询结果排序方式) |
    | symbol | string | BTC-USDT | N | 要查询的交易对，不传则不筛选 |
...
1. Params:
    ...
    | Field | Type | Example | Necessary | Desc |
    | --- | --- | --- | --- | --- |
    | bill_type | string |  |  | 要查询的[账单类型](./枚举说明.md#账单类型)，不传则不筛选 |
